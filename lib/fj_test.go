...
func TestFJBasic(t *testing.T) {
	...
	allYes := MockNameTracker{Paths: []string{"a", "b", "c", "d", "e", "e1", "e2", "f"}}
	f := NewFJ()
	...
	f = NewFJ()
	justAF := MockNameTracker{Paths: []string{"a", "f"}}
...
func TestFJ10Lines(t *testing.T) {
	...
	testTrackerSelection(NewFJ(), "L0", "../test_data/cl-sample-0",
		&MockNameTracker{Paths: []string{"type", "geometry"}},
	...
	testTrackerSelection(NewFJ(), "L1", "../test_data/cl-sample-1",
		&MockNameTracker{Paths: []string{"coordinates", "geometry"}},
		coordNames, coordVals, t)

	l2names := []string{"properties\nFROM_ST", "properties\nODD_EVEN"}
	l2vals := []string{`"1917"`, `"O"`}
	testTrackerSelection(NewFJ(), "L2", "../test_data/cl-sample-2",
		&MockNameTracker{Paths: []string{"properties", "FROM_ST", "ODD_EVEN"}},
		l2names, l2vals, t)
}
...
func testTrackerSelection(fj Flattener, label string, filename string, nt NameTracker, wantedPaths []string, wantedVals []string, t *testing.T) {
	event, err := ioutil.ReadFile(filename)
	if err != nil {
		t.Error(filename + ": " + err.Error())
	}

	list, err := fj.Flatten(event, nt)
	if err != nil {
...
type MockNameTracker struct {
	Paths []string
}

func (nt *MockNameTracker) IsNameUsed(name []byte) bool {
	for _, n := range nt.Paths {
		if string(name) == n {
			return true
		}
	}
